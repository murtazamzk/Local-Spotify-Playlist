...
function App() {
  ...
  useEffect(() => {
    const api_data = getParamValues(window.location.hash);
    if(api_data.access_token){
      const access_token = api_data.access_token;
      const expiry_time = new Date().getTime() + api_data.expires_in * 1000;
      setAccessToken(access_token);
      setExpiryTime(expiry_time);
      localStorage.setItem('@accessToken',access_token);
      localStorage.setItem('@expiryTime',expiry_time);
    }
  }, []);
  ...
  return (
    <StoreContext.Provider value={{ state, dispatch }}>
      <div className="app">
        <svg className="logo" viewBox="0 0 63 20" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMin meet">
          <g fillRule="evenodd">
        ...
        {
          isValidSession() ? <Playlists />:
          <button onClick={handleLogin} className="login-btn">Login to spotify</button>
        }
      </div>
